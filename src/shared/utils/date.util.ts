import { DEFAULT_FORMAT_DATE, TIMEZONE_UTC } from '@shared/constants';
import { FlexibleDate } from '@shared/types';
import {
...
export class DateUtil {
  ...
  static timeZoneToUTC(date: FlexibleDate, tz = TIMEZONE_UTC) {
    try {
      const dateUtc = fromZonedTime(date, tz);
  ...
  static toFormatUTC(localDate: FlexibleDate, format = DEFAULT_FORMAT_DATE) {
    ...
    return formatInTimeZone(dateISO, TIMEZONE_UTC, format);
  }
